...
:py:class:`~pyxu.opt.solver.reg_param_est.RegParamMAP` that estimates regularization parameters via
...
:math:`L_{2, 1}` norm :py:class:`~pyxu.operator.func.norm.L21Norm` and :math:`\nabla` is the
:py:class:`~pyxu.operator.linop.diff.Gradient` operator. The posterior distribution is thus given by
...
We thus apply the :py:class:`~pyxu.opt.solver.reg_param_est.RegParamMAP` algorithm with the objective functional
...
.. code-block:: python3
    ...
    from pyxu.abc import Mode
    import pyxu.operator as pycop
    import pyxu.opt.solver as pycsol
    import pyxu.opt.stop as pycstop
    from pyxu.opt.solver.reg_param_est import RegParamMAP
...
In the next example, we showcase another algorithm :py:class:`~pyxu.opt.solver.reg_param_est.RegParamMLE` that
...
We thus apply the :py:class:`~pyxu.opt.solver.reg_param_est.RegParamMLE` algorithm with the objective functional
...
.. code-block:: python3
    ...
    import pyxu.opt.stop as pycstop
    import pyxu.opt.solver as pycsol
    from pyxu.opt.solver.reg_param_est import ProxFuncMoreau, RegParamMLE
...
import pyxu.operator as pyco
import pyxu.runtime as pycrt
import pyxu.experimental.sampler as pycs
...
class RegParamMAP(pyca.Solver):
    ...
    **Remark 2:**
    ...
    :py:class:`~pyxu.opt.solver.pds.CP` solver, in which case we would have ``reg_key="h"``.
    ...
    **Initialization parameters of the class:**
    ...
    g: Func | None
        Regularization functional :math:`\mathcal{G}`, instance of :py:class:`~pyxu.abc.operator.Func`.
    reg_key: str
        ...
        (see `Remark 2`).
    solver: Solver
        Inner-loop solver, instance of :py:class:`~pyxu.abc.solver.Solver`
    kwargs_solver: dict
...
class RegParamMLE(pyca.Solver):
    ...
    * :math:`\mathcal{G}_k:\mathbb{R}^N\rightarrow \mathbb{R}` for all :math:`0 \leq k \leq K-1` are *convex*,
    ...
    approximated using the Monte-Carlo Markov chain (MCMC) method :py:class:`~pyxu.sampler.sampler.ULA` targeting the
    ...
    * :math:`\mathbf{x}_{n, s} \in \mathbb{R}^N` for :math:`n \in \mathbb{N}` and :math:`0 \leq s \leq S-1` are samples
      of a :py:class:`~pyxu.sampler.sampler.ULA` Markov chain targeting the posterior distribution
    ...
    **Remark 3:**
    ...
    utility class :py:class:`~pyxu.opt.solver.ProxFuncMoreau`. The gradient then calls that of the Moreau-Yosida
    ...
    **Remark 5:**
    ...
    A new :py:class:`~pyxu.sampler.sampler.ULA` chain is initialized at every iteration :math:`n` to account for the
    ...
    **Initialization parameters of the class:**
    ...
    g: DiffFunc | list[DiffFunc]
        ...
        :math:`\mathcal{G}_k` is an instance of :py:class:`~pyxu.abc.operator.DiffFunc`.
    ...
    **Parameterization** of the ``fit()`` method:
    ...
    gamma: Real
        Discretization step of ULA (see `Notes` of :py:class:`~pyxu.sampler.sampler.ULA` documentation`).
    batch_size: int
    ...
    def __init__(
        ...
        **kwargs,
        ...
        kwargs.update(
            log_var=kwargs.get("log_var", ("theta","x")),
        )
        super().__init__(**kwargs)
...
class ProxFuncMoreau(pyca.ProxDiffFunc):
    ...
    be useful within solvers that require differentiable functionals, notably :py:class:`~pyxu.opt.solver.RegParamMLE`.
    """
